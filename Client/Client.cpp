...
void Client::login(serverInfo server, userInfo user)
{
    if (server.password == user.password)
    {
        if (!_alreadyRegistered) {
            ...
            Message::send(_fd.fd, ":" + server.name + " 002 " + user.nickName + " Your host is *, running version " +
                                  server.version + "\r\n");
            Message::send(_fd.fd, ":" + server.name + " 003 " + user.nickName + " This server was created " +
                                  server.createDate + "\r\n");
            Message::send(_fd.fd, ":" + server.name + " 004 " + user.nickName + " * " + server.version +
                                  " Available user modes: , Available channel modes: biklot\r\n");
            _login = true;
        }
    }
    else
